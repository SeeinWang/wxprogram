...
if($_GET['action'] == "logout"){
    ...
    echo '<p>Successfully Log Out</p>
          <a href="visaMemberUserLogin.php"><button style="margin-top:20px">Back to login page</button></a>';
    exit;
}
...
if(!isset($_SESSION['user_id'])){
  if(isset($_POST['submit'])){//用户提交登录表单时执行如下代码
    ...
    if(!empty($user_username)&&!empty($user_password)){
      ...
      $query = "SELECT username FROM member_user WHERE username = '$user_username' AND "."password ='$user_password'";
      //用用户名和密码进行查询
      ...
      if(mysqli_num_rows($data)==1){
        ...
        $home_url = 'visaTeamData.php?user='.$user_username.'';
